...
import { Linkedin, Instagram, Twitter, Mail, ShieldAlert, Zap, Sparkles } from "lucide-react";
...
export const TeamSection: React.FC = () => {
  ...
  return (
    <section id="team" className="relative z-10 py-20 sm:py-32 bg-canvas overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10 sm:space-y-12 relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 font-mono text-xs text-amber tracking-widest uppercase">
            ...
            <span>SUPERHERO LEADERSHIP ROSTER</span>
          </div>

          <h2 className="text-3xl sm:text-5xl lg:text-6xl font-sans font-semibold text-text-heading tracking-tight">
            The Superhero Architects.
          </h2>

          <p className="text-text-body text-sm sm:text-lg leading-relaxed max-w-2xl mx-auto">
            From the Batcave to Krypton — our superhero nodal officers, student directors, and engineering leads guiding Holy Grace ventures.
          </p>
        </motion.div>

        {/* 3D Perspective Coverflow Carousel with Marquee Drift & Pointer Control */}
        <motion.div
        ...
        {/* Quick Superhero Selection Grid */}
      ...
      {/* Superhero Profile Modal with LinkedIn, Instagram, X/Twitter, Email */}
      <Modal
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              <img
                ...
                className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl object-cover border border-amber/40 shadow-xl flex-shrink-0"
              />
              <div className="min-w-0">
                <div className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded bg-amber/10 border border-amber/30 text-[10px] font-mono text-amber mb-1">
                  <ShieldAlert className="w-3 h-3" />
                  <span>VERIFIED SUPERHERO</span>
                </div>
                <h3 className="text-lg sm:text-xl font-sans font-bold text-text-heading truncate">
                  {selectedMember.name}
                </h3>
            ...
            {/* Superhero Dossier Bio */}
            <div className="space-y-2">
              <h4 className="font-mono text-xs text-amber tracking-wider uppercase">// SUPERHERO DOSSIER & BIO</h4>
            ...
            {selectedMember.contributions && (
              <div className="space-y-2">
                <h4 className="font-mono text-xs text-amber tracking-wider uppercase">// SUPERPOWERS & KEY FEATS</h4>
                <ul className="space-y-1.5 text-xs font-mono text-text-body">
                  {selectedMember.contributions.map((c, i) => (
                    <li key={i} className="flex items-start gap-2">
                      <Zap className="w-3.5 h-3.5 text-amber flex-shrink-0 mt-0.5" />
                      <span>{c}</span>
                    </li>
                  ))}
                </ul>
            ...
            {/* Superhero Telemetry Metrics Grid */}
            ...
            {/* Social Buttons: LinkedIn, Instagram, Twitter, Email */}
            <div className="pt-3 border-t border-border-subtle flex flex-wrap items-center gap-2 sm:gap-3">
              ...
              <a href={selectedMember.instagram} target="_blank" rel="noreferrer">
                <Button variant="secondary" size="sm" icon={<Instagram className="w-3.5 h-3.5 text-[#E4405F]" />}>
                  Instagram
                </Button>
              </a>

              <a href={selectedMember.twitter} target="_blank" rel="noreferrer">
                <Button variant="secondary" size="sm" icon={<Twitter className="w-3.5 h-3.5 text-[#1DA1F2]" />}>
                  X / Twitter
                </Button>
              </a>
